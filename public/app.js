...
elements.customerLookupBtn?.addEventListener("click", async () => {
  const rollId = document.getElementById("customer-lookup").value.trim();
  if (!rollId) {
    showToast("Enter a Roll ID.");
    return;
  }
  try {
    const data = await fetchRollById(rollId);
    elements.customerRollDetails.textContent = `Roll ${data.roll_id} is ${data.status}.`;
  } catch (error) {
    showToast(error.message);
  }
});
